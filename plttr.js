...
e.onclick = function (){
	...
	updateDisplay();
	updateNumber();
	infix.push(Math.E); //pushing the value instead of string. note: for other numbers, the string is pushed. this lets us check things like "12.3.4" later. we want to reserve all validating for after the "submit" button is pressed
}
...
//we changed the dropdowns to be horizontal axis and veritical axis, which are more inclusive and accurate labels but here we got lazy and left it as independent and dependent
...
function drawGraphLines(rangeObjectx, rangeObjecty, canvasWidth, canvasHeight, border, x, lineAdjustment, axisLabel) //x = true means drawing vertical lines, else hoizontal 
{
	...
	//first find where the first line will go - want any extra pixels to be on the side of the numbers, so we count down from the max
	while (curr > min)
	{
		curr -= increment; 
	}
...
clearEverything.onclick = refresh; //shared fate says this will work - need to verify
