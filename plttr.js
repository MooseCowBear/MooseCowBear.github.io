...
//first, adding a click event for fit selection buttons that will display the dropdowns. would prefer to have this as a hover effect, but that does not work for touch devices. 

document.body.addEventListener("click", function(e){ 
	
	if (e.target.nodeName === "BUTTON" && e.target.classList.contains("fit-button"))
	{
		//if they clicked a button, want to close any open dropdown other than the one belonging to the button if they are open
		const parent = e.target.parentNode;

		const dropdowns = document.querySelectorAll(".dropdown-content"); //get all the dropdowns
		dropdowns.forEach(elem =>
		{
			const dropdownParent = elem.parentNode;
			if (dropdownParent != parent && elem.classList.contains("show")) //if these dropdowns don't share a parent with the button (and so aren't the clicked button's dropdown, and they were visible, disappear them
			{
				elem.classList.remove("show");
			}
		});

		//then toggle the "show" class from the dropdown belonging to this clicked button
	...
	else
	{
		...
		//want to close any open dropdown as long as the click was not IN a dropdown, i.e. they weren't making a selection bc that is being handled below...
		{
